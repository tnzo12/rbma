...
mbmap <- function(x = MBMA.stan,...) {
  ...
  r = as.vector( x$data_wide %>%  select( grep("r[1-9]",x$data_wide %>% names()) ) %>% unlist() ) %>% as.numeric()
  dose = as.vector(  x$data_wide %>% select( grep("d[1-9]",x$data_wide %>% names()) ) %>% unlist() ) %>% as.numeric()
  n = as.vector(  x$data_wide %>% select( grep("n[1-9]",x$data_wide %>% names()) ) %>% unlist() ) %>% as.numeric()
  obs.probs = r/n
  
  mydata = data.frame(dose = dose,
                      y.obs = obs.probs,
                      y.samplesizes = n,
                      Study = gl(nrow(x$data_wide), x$data_wide %>% select( grep("d[1-9]",x$data_wide %>% names()) ) %>% ncol(),
